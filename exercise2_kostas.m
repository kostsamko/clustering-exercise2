...
%and 5 because they are highly correlated with 8 and 9
Countrydata(:,1) = [];
Countrydata(:,5) = [];
...
for j=2:m
    ...
    for t=1:run_times
        for k=1:j
            for i=1:l
                random_value = min_min_max_normalization(:,i) + (max_min_max_normalization(:,i)-min_min_max_normalization(:,i)) * rand(1,1);
                theta_ini(i,k) = random_value;
            end    
        end
    [theta,bel,J] = k_means(X, theta_ini);
